...
exp_file = file.path(scape_path, GSE, GSM, "pasite.csv.gz")
names(exp_file) = GSM
collapse_pa = file.path(scape_path, GSE, GSM, "collapse_pa.tsv.gz")
...
# write.csv(annot_info,file.path(scape_path, GSE, GSM, "APA_annotations.csv"))   ###################################  write
dir.create(file.path(scape_path,"spatial_h5ad/apa/rds",dataset,samp), recursive = TRUE, showWarnings = FALSE)
write.csv(annot_info,file.path(scape_path,"spatial_h5ad/apa/rds",dataset,samp,"APA_annotations.csv"))   ###################################  write

# ### 保存稀疏矩阵
# saveRDS(pa_mtx_fi, file = file.path(scape_path,GSE,GSM,"pa_mtx_fi.rds"))
