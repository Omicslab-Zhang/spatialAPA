...
spatial_dir <- file.path(scape_path, GSE, GSM)
...
dir.create(file.path(scape_path,"tool","diff_information",dataset,samp), recursive = TRUE, showWarnings = FALSE)
write.csv(top_features_rna,file.path(scape_path,"tool","diff_information",dataset,samp,"markers_top_Spatial.csv"),row.names=FALSE)   ###################################  write


########  cluster for apa
annot_info = read.csv(file.path(scape_path,"spatial_h5ad/apa/rds",dataset,samp,"APA_annotations.csv"),row.names=1)
pa_mtx_fi = readRDS(file.path(scape_path,GSE,GSM,"pa_mtx_fi.rds"))
...
write.csv(top_features_apa,file.path(scape_path,"tool","diff_information",dataset,samp,"markers_top_apa.csv"),row.names=FALSE)   ###################################  write
...
# 写入JSON文件
dir.create(file.path(scape_path,"spatial_h5ad/apa/overview",dataset,samp), recursive = TRUE, showWarnings = FALSE)
write(json_output, file = file.path(scape_path,"spatial_h5ad/apa/overview", dataset,samp,"APA_counts.json"))     ###################################  write
##### 转json对象 top APA events  #####
...
ggsave(file.path(scape_path, "spatial_h5ad/apa/overview", dataset,samp, "overview_7_apa_catego.pdf"), plot = p1,width=10,height=6)     ###################################  write
###### figure ==  overview - APA_Category  ######
...
write.csv(top_features_apapsi,file.path(scape_path,"tool","diff_information",dataset,samp,"markers_top_apapsi.csv"),row.names=FALSE)   ###################################  write
...
#######  cluster for apalen
pa_len_mtx <- read.csv(file.path(scape_path, GSE, GSM, "gene_apa_length_mtx.csv"), check.names=FALSE)
...
if(dim(filtered_pa_len_mtx)[1]!=0){
  ...
  write.csv(top_features_apalen,file.path(scape_path,"tool","diff_information",dataset,samp,"markers_top_apalen.csv"),row.names=FALSE)   ###################################  write
  ...
  sceasy::convertFormat(gene_obj, from="seurat", to="anndata", assay = select_assay,
                      outFile=file.path(scape_path, "spatial_h5ad/apa/rds",dataset,samp, paste0("anndata_",select_assay,".h5ad"))) ###################################  write (checked)
}
...
saveRDS(gene_obj,file.path(scape_path,"spatial_h5ad/apa/rds",dataset,samp,"spatial_data.rds"))   ###################################  write (checked)

RDS_file = file.path(scape_path,"spatial_h5ad/apa/rds",dataset,samp, "spatial_data.rds")  ################################################
...
if(dim(filtered_pa_len_mtx)[1]!=0){
  ...
  ggsave(file.path(scape_path, "spatial_h5ad/apa/overview", dataset, samp, "overview_1_QC.pdf"), plot = plot_all, width = 12, height = 6) ###################################  write
}else{
  plot_all <- plot_a + plot_b + plot_c + plot_e + plot_f + plot_g + plot_layout(nrow=2, ncol = 3) + 
    theme(plot.margin = unit(c(0.05, 0.05, 0.05, 0.05), "cm"))
  ggsave(file.path(scape_path, "spatial_h5ad/apa/overview", dataset, samp, "overview_1_QC.pdf"), plot = plot_all, width = 10, height = 6)
}
...
if(dim(filtered_pa_len_mtx)[1]!=0){
  ...
  ggsave(file.path(scape_path, "spatial_h5ad/apa/overview", dataset, samp, "overview_2_cluster.pdf"), plot = plot_all, width = 12, height = 6) ###################################  write
}else{
  plot_all <- plot_1 + plot_2 + plot_3 + plot_5 + plot_6 + plot_7 + plot_layout(nrow=2, ncol = 3)
  ggsave(file.path(scape_path, "spatial_h5ad/apa/overview", dataset, samp, "overview_2_cluster.pdf"), plot = plot_all, width = 10, height = 6) ###################################  write
}
############################################################ overview_2 cluster ############################################################
...
#### 读取top100差异基因列表
top_features_path = file.path(scape_path,"tool","diff_information",dataset,samp)
...
if(dim(filtered_pa_len_mtx)[1]!=0){
  ...
  pdf(file = file.path(scape_path, "spatial_h5ad/apa/overview", dataset, samp, "overview_3_heatmap.pdf"), width = 18, height = 13) ###################################  write
...
if(dim(filtered_pa_len_mtx)[1]!=0){
  ...
  ggsave(file.path(scape_path, "spatial_h5ad/apa/overview", dataset, samp, "overview_4_dotplot.pdf"), plot = plot_all, width = 18, height = 20) ###################################  write
}else{
  plot_all <- p_gene + p_apa + p_psi + plot_layout(nrow=3, ncol = 1)
  ggsave(file.path(scape_path, "spatial_h5ad/apa/overview", dataset, samp, "overview_4_dotplot.pdf"), plot = plot_all, width = 18, height = 15) ###################################  write
}
...
if(dim(filtered_pa_len_mtx)[1]!=0){
  ...
  p <- ggplot(apalen_data_filtered, aes(x = as.factor(cluster), y = log2_value, fill = as.factor(cluster))) +
    ...
    theme(plot.title = element_text(hjust = 0.5, size = 20, face = "bold"),
          ...
          panel.grid.minor = element_blank())
  ggsave(file.path(scape_path, "spatial_h5ad/apa/overview", dataset, samp, "overview_5_APAlength.pdf"), plot = p, width = 10, height = 8)
}
############################################################ overview_5 WUL_boxplot ############################################################
...
output_dir_cluster <- file.path(scape_path,"tool","cluster_information", dataset)
...
for (select_mtx in assays) {
    df <- read.csv(file.path(scape_path,"tool","diff_information", dataset, samp, paste0("markers_top_", select_mtx, ".csv")))
    # Get unique clusters
    ...
    output_dir <- file.path(scape_path,"tool","diff_information", dataset, samp)
    dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)
    # Save JSON file
...
for(select_assay in assays){
  list_dat <- read.csv(file.path(scape_path,"tool","diff_information", dataset, samp,paste0("markers_top_",select_assay,".csv")))
  # outpath = "/root/projects/spatialapadb/tool/enrich_information"
  outpath = file.path(scape_path,"tool","enrich_information")
